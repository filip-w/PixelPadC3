...
// Terminal Variables
int cursorY = 0;
const int lineHeight = 10; // Adjust based on text size (Size 1 is ~8-10 pixels)
...
void setup()   {
 ...
 //display.setContrast (0); // dim display
  ...
  terminalPrint("PixelPadC3");

  WiFi.STA.begin();
  Serial.println("Setup done");
  terminalPrint("Wifi Setup done");
}
...
void loop() {
  ...
  while(customKeypad.available()){
    ...
    // Convert the key bit to a char
      char keyChar = (char)e.bit.KEY;
    Serial.print(keyChar);
    if(e.bit.EVENT == KEY_JUST_PRESSED){
      Serial.println(" pressed");
      terminalPrint("Key: " + String(keyChar));

      // Trigger WiFi scan when '*' is pressed
      if(keyChar == '*'){
        terminalPrint("Scanning WiFi...");
        ScanWiFi();
      }
    } 
...
void ScanWiFi() {
  ...
  terminalPrint("Scan start..."); // Notify user on display 
  ...
  if (n == 0) {
    ...
    terminalPrint(String(n) + " nets found:");
...
/**
 * terminalPrint
 * Prints a string and handles automatic vertical scrolling
 */
void terminalPrint(String text) {
  ...
  // Move the "virtual" cursor down for the next call
  cursorY += lineHeight;
}
